				...
				<div class="table-responsive">
					<table class="table">
						...
						<tr v-for="(ip,$index) in ip_pagination.ips.data">
							...
							<td>
								<span v-if="!ip.editing">{{ ip.mac !== '' ? ip.mac : 'Not Assigned' }} <i class="fas fa-edit" style="cursor:pointer" @click.prevent="$set($data.ip_pagination.ips.data[$index],'editing',true)"></i></span>
								<span v-if="ip.editing">
                                    <div class="input-group">
                                        <input type="text" class="form-control" placeholder="00:16:3e:59:8b:f8"
											   v-model="ip.mac">
                                        <div class="input-group-addon">
                                            <button class="btn" type="button" @click.prevent="update_mac(ip,$index)" :disabled="ip.update_processing">
				...
				<div class="ipages">
					<div class="btn-group dropup">
						...
						<ul class="dropdown-menu">
							<li :class="{active: ip_pagination.per_page === 20}"><a href="#" @click.prevent="$set($data.ip_pagination,'per_page',20)">20 entries</a></li>
							<li :class="{active: ip_pagination.per_page === 50}"><a href="#" @click.prevent="$set($data.ip_pagination,'per_page',50)">50 entries</a></li>
							<li :class="{active: ip_pagination.per_page === 100}"><a href="#" @click.prevent="$set($data.ip_pagination,'per_page',100)">100 entries</a></li>
						</ul>
					</div>
					<div class="btn-group dropup">
						<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"
