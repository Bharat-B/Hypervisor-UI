<template>
	<div class="content animated fadeIn">
		<h2><i class="fa fa-layer-group" aria-hidden="true"></i>Create Instance Plan</h2>
		<div class="col-md-6">
...
export default {
	...
	mounted() {
		...
		$('[name="disk_driver"]').select2({placeholder: 'Select Driver'}).on('change', function () {
			...
			let overage = $('[name="bandwidth_overage"]');
			if (overage.hasClass('select2-hidden-accessible')) {
				overage.val('').select2('destroy');
			}
			vm.$nextTick(() => {
				$('[name="bandwidth_overage"]').select2({placeholder: 'Overage Action'});
			});
		...
		if (this.duplicate.id) {
			for (let key in this.duplicate) {
				if (this.duplicate.hasOwnProperty(key)) {
					if (['regions', 'image_groups', 'upgrades', 'hidden'].indexOf(key) === -1) {
						$('[name="' + key + '"]').val(this.duplicate[key]).trigger('change');
					}
					if (key === 'hidden') {
						$('[name="' + key + '"]').attr('checked', this.duplicate[key] === 1);
					} else if (key === 'regions') {
						...
						this.duplicate[key].forEach((upgrade) => {
							upgrades.push({id: upgrade.id, text: upgrade.name, selected: true});
						});
					}
				}
			}
